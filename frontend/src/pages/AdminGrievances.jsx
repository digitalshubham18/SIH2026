...
import { LifeBuoy, Clock, AlertTriangle, CheckCircle2, XCircle, EyeIcon, Filter, Scale, Award, CreditCard, HelpCircle, ShieldCheck } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50/40 via-white to-amber-50/30">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        <div className="mb-6">
          <div className="text-xs uppercase tracking-widest text-emerald-700 font-semibold flex items-center gap-1"><ShieldCheck className="w-3 h-3" />Officer Console</div>
                ...
                return (
                  <div key={g.id} data-testid={`grv-row-${g.id}`} className={`p-4 rounded-xl border ${g.sla_breached ? "border-rose-300 bg-rose-50/30" : "border-emerald-100 bg-white"} flex items-start gap-4`}>
                    ...
                    {(g.status === "open" || g.status === "in_review") && (
                      <Button data-testid={`grv-respond-${g.id}`} onClick={() => openRespond(g)} size="sm" className="bg-emerald-700 hover:bg-emerald-800 text-white">
                        Respond
                      </Button>
                    )}
                  </div>
